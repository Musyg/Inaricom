...
// Page de test isolée pour MatrixRainRed.
// 5 sections côte à côte, 1 seule a data-theme="rouge" → canvas attendu.
// Les 4 autres (or, vert, bleu, neutre) ne doivent rendre AUCUN canvas (return null).
...
function TestSection({
  ...
  // Lit la valeur hex de --inari-red telle que calculée pour ce thème.
  // Permet au badge d'afficher la vraie couleur (pas "var(--inari-red)" en dur).
  useLayoutEffect(() => {
    ...
    <main className="min-h-screen bg-inari-black text-inari-text">
      <header className="border-b border-inari-border px-6 py-5">
        <h1 className="font-serif text-2xl text-inari-white">
          Test · MatrixRainRed <span className="text-inari-accent">(thème rouge uniquement)</span>
        </h1>
        <p className="mt-2 max-w-3xl text-sm text-inari-text-soft">
          Attendu : animation visible uniquement dans la colonne{' '}
          <code className="font-mono text-inari-accent">data-theme="rouge"</code>. Les 4 autres
      ...
      <div className="grid grid-cols-1 gap-px bg-inari-border lg:grid-cols-5">
        <TestSection
          ...
          description="Canvas attendu en rouge Inari"
        />
...
const root = document.getElementById('test-matrix-rain-root')
if (root) {
