 ...
 * Background parallax avec faisceaux lumineux indépendants
 * Version: 1.0.0 (basé sur v17)
 */
...
(function() {
    ...
    function stitchSegments(segs, threshold) {
        ...
        for (const color in byColor) {
            ...
            colorSegs.filter(s => !s.merged).forEach(s => {
                result.push({ points: s.points, color: s.color, parentId: s.parentId });
            });
        }
        return result;
    }
    ...
    class PulseBeam {
        constructor(segment, id) {
            ...
            this.id = id;
            this.color = CONFIG.colors[segment.color] || CONFIG.colors['#ffffff'];
        ...
        update(deltaTime) {
            if (this.state === 'running') {
                ...
                if (this.trail.length > 0 && this.pauseTimer < CONFIG.pauseDuration * 0.3) {
                    ...
                    for (let i = 0; i < pointsToRemove && this.trail.length > 0; i++) {
                        this.trail.shift();
                    }
                }
    ...
    // Variables globales
    let canvas, ctx, segments = [], beams = [], lastTime = 0;
    let animationId = null;

    function resizeCanvas() {
        const container = document.getElementById('fox-canvas-container');
        if (!container || !canvas) return;
        
        // Conserver le ratio du SVG original
        const svgRatio = canvas.dataset.originalWidth / canvas.dataset.originalHeight;
    ...
    async function loadPaths() {
        try {
            // URL du JSON passée par WordPress
            const jsonUrl = (typeof foxAnimationData !== 'undefined') 
                ? foxAnimationData.jsonUrl 
                : 'fox-paths.json';
            ...
            canvas.dataset.originalWidth = canvasW;
            canvas.dataset.originalHeight = canvasH;
    ...
    function init() {
        canvas = document.getElementById('fox-canvas');
        if (!canvas) return;
